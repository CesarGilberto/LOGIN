    ...
    if (this.formRegister.valid) {
      if (this.formRegister.get('password').value === this.formRegister.get('r_password').value) {
        this.userService.addUser({
          ...
          gallery: [
            {photo: 'assets/img/avatar.png', description: '1'},
            {photo: 'assets/img/avatar.png', description: '2'},
            {photo: 'assets/img/avatar.png', description: '3'}
          ],
          active: true
        });
        this.tool.showMessageToast('REGISTRO EXITOSO');
        this.router.navigate(['/home']);
      } else {
        this.tool.showMessage('ALERTA', 'LAS CONTRASEÑAS NO COINCIDEN');
      }
    } else {
      this.tool.showMessage('ALERTA', 'VALORES INVÁLIDOS');
    }
  }
